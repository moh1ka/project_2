...
int enhance_40 = 1;
int enhance_41 = 0;
int enhance_3 = 0;//makes things much worse
int enhance_42 = 0;
int enhance_43 = 0;
int enhance_44 = 0;
...
//void enh41 (float *min_x, float *x);
//void enh42 (float *min_x, float *x);
//void enh44 (float *min_x, float *x)
/********************************** Main routine ************************************/
void main()
{      
...
{	
	...
	for  (i=0; i<FFTLEN;i++)
	{
		pbuf[i] = ( (1-lpf_k) * x[i] ) + ( lpf_k * pbuf[i] );
		//x[i] = p[i];
	}
	//return pbuf;
	//G[i] = max( lambda,  ( 1 - (min_x[i]/p[i]) ) );
...
void enh41 (float *min_x, float *x)
{
	G[i] = max( lambda * (min_x[i]/x[i]) ,  ( 1 - (min_x[i]/x[i]) ) );
}

void enh42 (float *min_x, float *x)
{
	
	G[i] = max( lambda * (p[i]/x[i]) ,  ( 1 - (min_x[i]/x[i]) ) );
}
